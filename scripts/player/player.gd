...
func _looks_at_wall(wall_check_top : RayCast2D, wall_check_bottom : RayCast2D) -> bool:
	...
	var c1 = wall_check_top.get_collider()
	var c2 = wall_check_bottom.get_collider()
	if c1 != c2:
		return false

	# normals should both be mostly horizontal and point the same way
	var n1 = wall_check_top.get_collision_normal()
	var n2 = wall_check_bottom.get_collision_normal()
	var same_dir = n1.dot(n2) > 0.9
	var horizontal = abs(n1.x) > 0.7 and abs(n1.y) < 0.5
