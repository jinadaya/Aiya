...
class UiDialog:
	...
	func _init(m_entity_name: String, m_text: String, m_previews: Array[String]):
...
func load_dialog_tree(path: String) -> bool:
	...
	var json_obj = JSON.parse_string(json_content)
...
func emit_current_dialog():
	...
	var entity_name = entities.get(current.entity, "Unknown")
	...
	var ui_dialog = UiDialog.new(entity_name, current.text, previews)
